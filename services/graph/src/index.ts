...
export async function ensureDatabase(db: Database, name: string) {
  ...
  if (!list.includes(name)) await db.createDatabase(name);
}
export async function ensureCollections(db: Database) {
  const existing = await db.listCollections();
  const names = new Set(existing.map(c => c.name));
  if (!names.has(COLLECTIONS.objects)) await db.collection(COLLECTIONS.objects).create();
  if (!names.has(COLLECTIONS.edges)) await db.collection(COLLECTIONS.edges).create({ type: 3 });
}
export async function upsertObject(db: Database, obj: GraphObject) {
  const col = db.collection(COLLECTIONS.objects);
  if (obj._key) { await col.update(obj._key, obj, { keepNull: false }); return obj._key; }
  const meta = await col.save(obj); return meta._key;
}
export async function createEdge(db: Database, edge: GraphEdge) {
  const col = db.collection(COLLECTIONS.edges);
  const meta = await col.save(edge); return meta._key;
}
...
export function recommendInMemory(objects: GraphObject[], edges: InMemoryEdge[], userKey: string, limit = 10): GraphRecommendation[] {
  ...
  const targets = edges.filter(e => e._from === fromIdPrefix && (e.type === 'recommends' || e.type === 'follows')).map(e => e._to);
...
export async function bootstrap() {
  ...
  if (cfg.username && cfg.password) sys.useBasicAuth(cfg.username, cfg.password);
  try {
    ...
    if (count.count === 0) {
      const u = await upsertObject(db, { type: 'user', name: 'Demo User' });
      const h = await upsertObject(db, { type: 'hub', name: 'Demo Hub' });
      await createEdge(db, { _from: `${COLLECTIONS.objects}/${u}`, _to: `${COLLECTIONS.objects}/${h}`, type: 'joins', createdAt: Date.now() });
    }
    ...
    console.warn('[graph] Bootstrap skipped:', (err as Error).message);
  }
}
