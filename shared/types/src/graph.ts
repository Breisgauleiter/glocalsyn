...
// Simple runtime validation for edges (useful in adapters / service layer)
...
export function getRecommendations(objects: GraphObject[], edges: GraphEdge[], userKey: string): GraphObject[] {
  const recommendedIds = edges
    .filter(e => e._from === `user/${userKey}` && (
      e.type === 'recommends' ||
      e.type === 'follows' || // treat follows as a weak recommendation source
      e.type === 'continues'  // narrative continuation (quest chains) when emanating from a user context
    ))
    .map(e => e._to);
  // De-duplicate while preserving order (earliest first)
